...
#Factory image name is : S-010W-A_SW_FAC_RTL8188FTV_R1.00.00.zip
...
#make kernel
    ...
    make rk3228b-box_805_tykh2l.img -j20
...
#make kernel
    ...
    make ARCH=arm64 rockchip_smp_kitkat_defconfig
    make ARCH=arm64 rk3228h-demo.img -j20
    echo "You've build kernel correctly"
#make android
    cd $WP
    make installclean
	cp -a $WP/kernel/arch/arm64/boot/Image $WP/out/target/product/rk3228h/kernel
    make -j16
    echo "you've make android correctly"
#build image
#only needed when official
...
#logo
    if [ "$carrier" == "cu" ]; then
        ...
        if [ "$province" == "nx" ]; then
            ...
            cp ${imgmake_tool}/logo_ctnm.bmp ${targetimg}/logo.bmp
...
MakebuildpropFac () {
local workpath=$HOME/build/input/base
local imgname="$workpath/$imgpfx.img"
local imgmd5="$workpath/$imgpfx.txt"
local otafullname="$workpath/$imgpfx.zip"
local sp=" "
local MD5="MD5:"
local Zip="zip"
local Poi="."
...
CalFacOtaParam () {
    ...
    imgpfx="$PRODUCTNAME$Con$SW$ConFAC$Con$VERSIONSTR$int"
...
MakeDiffFile(){
    ...
    echo "diffFile_ret:"$diffFile_ret
