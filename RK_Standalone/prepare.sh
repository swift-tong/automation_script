...
FormaliseIptvApk () {
    ...
    SubstrMatch2 $filename "IPTV" "Setting"
    if [ $? -eq 0 ]; then
...
FormaliseFileManagerApk()
{
	echo "++++ FileManager now"
    local filename="$1"
    local target="FileManager.apk"
    SubstrMatch $filename "FileManager"
    if [ $? -eq 0 ]; then 
        echo "formailise it to FileManager.apk"
        target="FileManager.apk"
        mv $filename $target
        return 0
    fi
    return 1
...
FormaliseSingleApk() {
	...
	if [ $? -eq 0 ]; then return 0 ; fi
	FormaliseFileManagerApk $filename
	if [ $? -eq 0 ]; then return 0 ; fi
	################end#####################
    return 1
}
