...
ReadCdeParameter () {
    ...
    appversion=$version #for launcher version
...
#first we should change device.mk
    ...
    #change field ro.vendor.sw.version to S-010W-A_SW_ZY_CTNX_R1.01.15
...
CheckBuildInfo () {
    ...
    if [ "$product" = "S-010W-AV2B" ] && [ "$carrier" = "os" ] && [ "$province" = "sm" ];then	
...
#ro.cm.stb.type S-010W-A
#ro.cm.stb.manufacturer refer ro.product.manufacturer
#ro.cm.stb.AppVersion fill Launcher APK version，driven by iptvcde_ctln.ini
#    appversion=
#ro.cm.stb.RomVersion ，external version
#ro.cm.stb.HardwareVersion ，refer to ro.build.hardware.id
MdSuyingMkCtln () {
    ##now append ro.cm.stb.type
    cd $iptv
    grep ro.cm.stb.type suying.mk
    if [ $? -ne 0 ]; then
    local append0="        ro.cm.stb.type=S-010W-A"
    echo $append0
    sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$append0 \\\\"'' suying.mk
	fi  

    grep ro.cm.stb.manufacturer suying.mk
    if [ $? -ne 0 ]; then
    local append1="        ro.cm.stb.manufacturer=NSB"
    echo $append1
    sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$append1 \\\\"'' suying.mk
	fi
    
    grep ro.cm.stb.AppVersion suying.mk
    if [ $? -ne 0 ]; then
        local append2="        ro.cm.stb.AppVersion=$appversion"
        echo $append2
        sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$append2 \\\\"'' suying.mk
    fi
    
    grep ro.cm.stb.RomVersion suying.mk
    if [ $? -ne 0 ]; then
        local append3="        ro.cm.stb.RomVersion=$productswversion"
        echo $append3
        sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$append3 \\\\"'' suying.mk
    fi  

    grep ro.cm.stb.HardwareVersion suying.mk
    if [ $? -ne 0 ]; then
        local append4="        ro.cm.stb.HardwareVersion=$producthwversion"
        echo $append4
...
MdRk3228bSuyingMk () {
    ...
    if [ $? -ne 0 ]; then
        if [ "$product" = "S-010W-A" ] && [ "$carrier" = "cu" ] && [ "$province" = "sd" ];then
    ...
    #append ro.sys.platform
    grep ro.sys.platform suying.mk
    if [ $? -ne 0 ]; then
        local append8="        ro.sys.platform=$inputlauncher"
        echo $append6
    ...
    #    sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$append7 \\\\"'' suying.mk
    #fi
    if [ "$carrier" == "ct" ] && [ "$province" == "ln" ]; then
        MdSuyingMkCtln
    fi
}
...
MdRk3228hSuyingMk () {
    ...
    grep ro.product.cpuarchitec suying.mk
    if [ $? -ne 0 ]; then
        local append6="        ro.product.cpuarchitec=ARM-A53"
        echo $append6
        sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$append6 \\\\"'' suying.mk
    fi
    ...
    if [ "$PRODUCTNAME" = "S-010W-A" ] && [ "$carrier" = "cu" ] && [ "$province" = "bj" ];then
        grep persist.sys.hwconfig.sw_ver suying.mk
        if [ $? -ne 0 ]; then
            local ap="        persist.sys.hwconfig.sw_ver=$extVersion"
            echo $ap
            sed -i '/PRODUCT_PROPERTY_OVERRIDES/a \'"$ap \\\\"'' suying.mk
        fi
    fi
...
CheckVersionReturn () {
    ...
    if [ $ret = 'lessargs' ];then
        ...
        echo " not accepted method like 'getExtVer 'or ‘getIntVer'"
        return 1
...
UpdateVersionStr () {
    ...
    versioncode=$(python $python_s/intmapext.py  "checkMajorversion" "$PRODUCTNAME" "$inputchip_u" "$inputmodule_u" "$middleware" "$inputlauncher" "$carrier$province" $majorversion $minorversion)
    CheckVersionReturn $versioncode
    if [ $versioncode == "wrongmajorversion" ];then
        echo "majorversion is not right,please check"
        return 1
    fi
...
BuildCheck(){
    ...
    if [ "$keyValueMapping_ret" == "False" ];then
        echo "keyValueMapping return false!! line 1150"
        flag=1
    fi
...
AddFactroyPatch () {
    ...
    patch -p1 < /home/mengxl/build/patch/patch/kernel/0003-cpy-to-nvram_ap6356.txt-on-BCM4356_CHIP_ID-BCM4371_C.patch 
